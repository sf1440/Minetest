...
std::shared_ptr<ModMetadata> ModApiStorage::get_mod_storage_cpp(const std::string& mod_name, IGameDef* gamedef)
{
    std::shared_ptr<ModMetadata> store = std::make_shared<ModMetadata>(mod_name);
    if (gamedef) {
        store->load(gamedef->getModStoragePath());
        gamedef->registerModStorage(store.get());
    }
    else {
        assert(false); // this should not happen
    }
